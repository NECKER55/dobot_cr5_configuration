...
void processCircumference(const custom_messages::msg::Object& plant, 
...
bool attemptToReachPoint(const custom_messages::msg::Object& plant,
...
void scanUncoveredPoints(const custom_messages::msg::Object& plant,
...
{
    RCLCPP_INFO(logger_, "========================================");
    RCLCPP_INFO(logger_, "Received map from reworked_map");
    RCLCPP_INFO(logger_, "Map contains %zu objects", msg->objects.size());
    ...
    RCLCPP_INFO(logger_, "Found %d target plants to scan", target_count);
    ...
    RCLCPP_INFO(logger_, "========================================");
    RCLCPP_INFO(logger_, "Map processing complete");
}

void addObstaclesToScene(moveit::planning_interface::MoveGroupInterface& move_group, moveit::planning_interface::PlanningSceneInterface planning_scene_interface)
{
    RCLCPP_INFO(logger_, "Adding obstacles to planning scene...");
    ...
    floor_pose.position.z = current_map_.work_space.low_left.z - 0.005;
    ...
    RCLCPP_INFO(logger_, "  Added workspace floor at z=%.3f", floor_pose.position.z);
    ...
    for (size_t i = 0; i < current_map_.objects.size(); ++i)
    {
        ...
        double size_x = object.shape.top_right.x - object.shape.low_left.x;
        double size_y = object.shape.top_right.y - object.shape.low_left.y;
        double size_z = object.shape.top_right.z - object.shape.low_left.z;
        ...
        RCLCPP_INFO(logger_, "  Added %s object_%zu: size=(%.3f, %.3f, %.3f) center=(%.3f, %.3f, %.3f)",
    ...
    rclcpp::sleep_for(1s);
    RCLCPP_INFO(logger_, "Successfully added %zu collision objects to scene", collision_objects.size());
}
...
void processTargetPlants(moveit::planning_interface::MoveGroupInterface& move_group)
{
    ...
    for (const auto& object : current_map_.objects)
    {
        ...
        {
            plant_number++;
            RCLCPP_INFO(logger_, "\n--- Processing Target Plant %d ---", plant_number);
            RCLCPP_INFO(logger_, "Plant bounding box:");
            RCLCPP_INFO(logger_, "  Low left: (%.3f, %.3f, %.3f)", 
                       object.shape.low_left.x, object.shape.low_left.y, object.shape.low_left.z);
            RCLCPP_INFO(logger_, "  Top right: (%.3f, %.3f, %.3f)", 
                       object.shape.top_right.x, object.shape.top_right.y, object.shape.top_right.z);
            RCLCPP_INFO(logger_, "Number of trajectories: %zu", 
            ...
            {
                RCLCPP_INFO(logger_, "\n  Processing trajectory %zu/%zu", 
                           traj_idx + 1, object.possible_trajectories.size());
                processCircumference(object, object.possible_trajectories[traj_idx],move_group);
            }
            
            RCLCPP_INFO(logger_, "--- Completed Plant %d ---\n", plant_number);
        }
    }
}
...
void processCircumference(const custom_messages::msg::Object& plant, 
                         ...
                         moveit::planning_interface::MoveGroupInterface& move_group)
{
    RCLCPP_INFO(logger_, "  Circumference has %zu points", circumference.circumference.size());
    ...
    for (size_t i = 0; i < circumference.circumference.size(); ++i)
    {
        ...
        if (sp.point.optimality == 1.0) optimal_count++;
        else if (sp.point.optimality == 0.0) unreachable_count++;
    }
    
    RCLCPP_INFO(logger_, "  Point statistics: %d optimal, %d unreachable", 
               optimal_count, unreachable_count);
    ...
    RCLCPP_INFO(logger_, "  Found %d boundary points", boundary_count);
    
    // Phase 1: Try to visit boundary points
    if (boundary_count > 0)
    {
        RCLCPP_INFO(logger_, "  Phase 1: Attempting to visit boundary points");
        ...
        for (size_t i = 0; i < scan_points.size(); ++i)
        {
            ...
            {
                RCLCPP_INFO(logger_, "    Attempting boundary point %zu at (%.3f, %.3f, %.3f)", 
                           i, scan_points[i].point.x, scan_points[i].point.y, scan_points[i].point.z);
                
                if (attemptToReachPoint(plant, scan_points[i], scan_points, move_group))
                {
                    visited_boundaries++;
                }
            }
        }
        RCLCPP_INFO(logger_, "  Phase 1 complete: visited %d/%d boundary points", 
                   visited_boundaries, boundary_count);
    }
    else
    {
        RCLCPP_INFO(logger_, "  No boundary points found, skipping Phase 1");
    }
    
    // Phase 2: Visit any remaining uncovered optimal points
    RCLCPP_INFO(logger_, "  Phase 2: Scanning remaining uncovered points");
    scanUncoveredPoints(plant, scan_points,move_group);
}
                        ...
                        moveit::planning_interface::MoveGroupInterface& move_group)
{
    ...
    geometry_msgs::msg::Point plant_center;
    plant_center.x = (plant.shape.low_left.x + plant.shape.top_right.x) / 2.0;
    plant_center.y = (plant.shape.low_left.y + plant.shape.top_right.y) / 2.0;
    
    if (first_scan)
    {
        plant_center.z = (plant.shape.low_left.z + plant.shape.top_right.z) / 2.0;
        first_scan = false;
    }
    else
    {
        plant_center.z = scan_point.point.z;
    }
    RCLCPP_DEBUG(logger_, "      Plant center: (%.3f, %.3f, %.3f)", 
                plant_center.x, plant_center.y, plant_center.z);
    
    // Create pose for the scan point
    geometry_msgs::msg::Pose target_pose = createPosePointingToCenter(
        scan_point.point, plant_center);
    ...
    if (success)
    {
        RCLCPP_INFO(logger_, "      ✓ Planning successful for point %d", scan_point.index);
        ...
        {
            RCLCPP_INFO(logger_, "      ✓ Successfully moved to scan point");
            ...
            RCLCPP_INFO(logger_, "      ✓ Marked %d points as covered by camera FOV", covered_count);
            
            rclcpp::sleep_for(500ms); // Allow time for scanning
            return true;
        }
        else
        {
            RCLCPP_ERROR(logger_, "      ✗ Execution failed with error code: %d", 
                        execution_result.val);
        }
    }
    else
    {
        RCLCPP_WARN(logger_, "      ✗ Planning failed for point %d", scan_point.index);
                        ...
                        moveit::planning_interface::MoveGroupInterface& move_group)
{
    ...
    RCLCPP_INFO(logger_, "    Found %zu uncovered points to scan", uncovered_indices.size());
    
    if (uncovered_indices.empty())
    {
        RCLCPP_INFO(logger_, "    All optimal points are covered!");
        return;
    }
    ...
    for (size_t idx : uncovered_indices)
    {
        ...
        {
            RCLCPP_INFO(logger_, "    Attempting uncovered point %zu at (%.3f, %.3f, %.3f)", 
                       idx, scan_points[idx].point.x, scan_points[idx].point.y, scan_points[idx].point.z);
            
            if (attemptToReachPoint(plant, scan_points[idx], scan_points, move_group))
            {
                successful_scans++;
            }
            else
    ...
    RCLCPP_INFO(logger_, "    Phase 2 complete: %d successful, %d failed", 
    ...
    RCLCPP_INFO(logger_, "  Final Coverage: %d/%d optimal points (%.1f%%)",
                covered, total_optimal, 
                total_optimal > 0 ? (100.0 * covered / total_optimal) : 0.0);
    ...
    if (covered < total_optimal)
    {
        ...
        for (const auto& sp : scan_points)
        {
            ...
            {
                RCLCPP_WARN(logger_, "    - Point %d at (%.3f, %.3f, %.3f)", 
...
int main(int argc, char **argv)
{
    ...
    RCLCPP_INFO(node->get_logger(), "Initializing MoveCR5Node...");
    ...
    move_group.setPlanningTime(5.0); // 5 seconds max planning time
    move_group.setNumPlanningAttempts(10); // Try up to 10 times
    ...
    RCLCPP_INFO(node->get_logger(), "MoveIt configuration:");
    RCLCPP_INFO(node->get_logger(), "  - Planning time: %.1f seconds", move_group.getPlanningTime());
    RCLCPP_INFO(node->get_logger(), "  - Position tolerance: %.3f m", move_group.getGoalPositionTolerance());
    RCLCPP_INFO(node->get_logger(), "  - Orientation tolerance: %.3f rad", move_group.getGoalOrientationTolerance());
    RCLCPP_INFO(node->get_logger(), "  - Planning frame: %s", move_group.getPlanningFrame().c_str());
    RCLCPP_INFO(node->get_logger(), "  - End effector link: %s", move_group.getEndEffectorLink().c_str());
    ...
    RCLCPP_INFO(node->get_logger(), "MoveCR5Node initialization complete, waiting for map...");
